...
1. **프론트엔드 Dockerfile 작성** (CHA-65)
   ```bash
   # frontend/Dockerfile 생성
   # frontend/.dockerignore 생성
   ```

2. **docker-compose 통합** (CHA-66)
   ```bash
   # 로컬에서 전체 스택 테스트
   docker-compose up --build
   ```

3. **PostgreSQL 마이그레이션** (CHA-67)
   ```bash
...
| ID | 제목 |
|----|------|
| CHA-65 | 프론트엔드 Dockerfile 작성 |
| CHA-66 | docker-compose 통합 구성 |
| CHA-67 | SQLite → PostgreSQL 마이그레이션 |
| CHA-68 | Azure Container Registry 설정 |
| CHA-69 | Azure Container Apps 배포 |
| CHA-70 | GitHub Actions CI/CD 파이프라인 |
